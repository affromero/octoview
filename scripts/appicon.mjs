...
// the archive. Renders assets/logo.svg on octoview's dark rounded-rect tile at
...
// macOS icon slots: size (pt) x scale -> pixel side. The system icon grid insets
// the glyph, so the logo fills ~62% of a rounded tile (Apple's ~22.37% corner
// radius superellipse, approximated with border-radius).
const SLOTS = [
  [16, 1],
  [16, 2],
  [32, 1],
...
const render = async (px) => {
  ...
  await writeFile(join(iconset, `icon_${px}.png`), await page.screenshot({ omitBackground: true }));
};
